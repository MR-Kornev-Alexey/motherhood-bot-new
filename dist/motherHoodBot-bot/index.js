...
motherHoodBot.start(async (ctx) => {
    try {
        await ctx.replyWithHTML(`<b>Добрый день ${ctx.message.from.first_name || 'незнакомец'}!</b>\n${start}`);
...
async function safeSendMessage(ctx, userId, text) {
    try {
    ...
    if (msg.chat.type === 'supergroup' && msg.reply_to_message?.forward_from_chat) {
        ...
        try {
            if (sent) {
                ...
                await ctx.reply(`${userName}, чтобы мы могли связаться с вами — пожалуйста, откройте бот.`, {
                    reply_markup: {
                        inline_keyboard: [[
